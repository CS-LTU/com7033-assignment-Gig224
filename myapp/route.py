...
def patient_edit(id):
    ...
    if form.validate_on_submit():  # Only prompt on POST with valid data
        # Update patient fields with form data
        form.populate_obj(patient)  # Transfer form data to patient fields directly
        db.session.commit()
...
def delete(id):
    ...
    try:
        ...
        db.session.commit()
        flash("Patient's data have been deleted successfully.", "success")
...
def register():
    ...
    if form.validate_on_submit():
        ...
        if User.query.filter_by(email=email).first():
            flash('Email already exist. Please choose a different one.', 'danger')
            return redirect(url_for('register'))
